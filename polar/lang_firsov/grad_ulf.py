...
def get_grad_lf(mylf, params=None, rdm1=None, mo_coeff=None, mo_occ=None,
    ...
    rdm1_tr = np.sum(rdm1, axis=0)

    rdm1_diag = rdm1_tr[range(nao), range(nao)]
    # 1-body

    grad_l = (w_p * lams - g - w_p * zs) * rdm1_diag
    h1 = np.array(mylf.get_h1(), copy=True)
    h1[range(nao), range(nao)] = 0.0
    grad_l += einsum('kj, k, k, j, jk -> k', h1, fac, -lams, fac, rdm1_tr)
    ...
    if h2 is not None:
        ...
        for i in range(nao):
            for j in range(nao):
                for k in range(nao):
                    for l in range(nao):
                        hr = h2[i, j, k, l] * (rdm1_tr[j, i] * rdm1_tr[l, k] -
                                               rdm1[0, l, i] * rdm1[0, j, k] -
                                               rdm1[1, l, i] * rdm1[1, j, k]) * 0.25
...
def get_grad_glf(mylf, params=None, rdm1=None, mo_coeff=None, mo_occ=None,
    ...
    # 1-body exp
    grad_l -= einsum('pm, pm, ym, mp -> ym', h1, fac, lams, rdm1_tr) * 2.0
    grad_l += einsum('pm, pm, yp, mp -> ym', h1, fac, lams, rdm1_tr) * 2.0
    ...
    if h2 is not None:
        ...
        # mmqq = mm, qq - (mq, mq)
        rdm2_diag = np.einsum('mm, qq -> mq', rdm1_tr, rdm1_tr) - \
                    np.einsum('Smq, Smq -> mq', rdm1, rdm1)
        ...
        tmp = -einsum("pmrs, ym, mp, sr -> ym", H2, lams, rdm1_tr, rdm1_tr)
        tmp += einsum("pmrs, ym, Sps, Smr -> ym", H2, lams, rdm1, rdm1)

        tmp += einsum("pmrs, yp, mp, sr -> ym", H2, lams, rdm1_tr, rdm1_tr)
        tmp -= einsum("pmrs, yp, Sps, Smr -> ym", H2, lams, rdm1, rdm1)

        tmp -= einsum("pmrs, ys, mp, sr -> ym", H2, lams, rdm1_tr, rdm1_tr)
        tmp += einsum("pmrs, ys, Sps, Smr -> ym", H2, lams, rdm1, rdm1)

        tmp += einsum("pmrs, yr, mp, sr -> ym", H2, lams, rdm1_tr, rdm1_tr)
        tmp -= einsum("pmrs, yr, Sps, Smr -> ym", H2, lams, rdm1, rdm1)
...
def get_grad_glf_2(mylf, params=None, rdm1=None, mo_coeff=None, mo_occ=None,
    ...
    # grad z term 0 4 11
    grad_z  = np.einsum('xpq, qp -> x', G, rdm1_tr)
    grad_z -= np.einsum('x, xp -> x', w_p, lams * rdm1_tr[range(nao), range(nao)])
    grad_z += (w_p * zs)
    grad_z *= 2.0
    ...
    tmp = np.einsum('pm, mp -> pm', h_tmp * fac1, rdm1_tr) * 2.0
    grad_l  = np.dot(lams, tmp)
    grad_l -= np.einsum('ym, pm -> ym', lams, tmp)

    # term 5 and 6 linear grad
    grad_l -= einsum('ypm, mp -> ym', G, rdm1_tr) * 2.0
    ...
    if h2 is not None:
        ...
        grad_l -= einsum("y, yq, Smq, Smq -> ym", w_p * 2.0, lams, rdm1, rdm1)
        ...
        vk = lib.einsum('pqrs, Sps -> Sqr', h_tmp, rdm1)

        #grad_l -= einsum('pmrs, ym, mp, sr -> ym', h_tmp, lams, rdm1, rdm1_2)
        grad_l -= einsum('pm, ym, mp -> ym', vj, lams, rdm1_tr)
        #grad_l += einsum('pmrs, ym, ps, mr -> ym', h_tmp, lams, rdm1, rdm1)
        grad_l += einsum('Smr, ym, Smr -> ym', vk, lams, rdm1) * 2.0

        #grad_l += einsum('pmrs, yp, mp, sr -> ym', h_tmp, lams, rdm1, rdm1_2)
        grad_l += einsum('pm, yp, mp -> ym', vj, lams, rdm1_tr)
        grad_l -= einsum('pmrs, yp, Sps, Smr -> ym', h_tmp, lams, rdm1, rdm1) * 2.0

        grad_l -= einsum('pmrs, ys, mp, sr -> ym', h_tmp, lams, rdm1_tr, rdm1_2)
        grad_l += einsum('pmrs, ys, Sps, Smr -> ym', h_tmp, lams, rdm1, rdm1) * 2.0

        grad_l += einsum('pmrs, yr, mp, sr -> ym', h_tmp, lams, rdm1_tr, rdm1_2)
        #grad_l -= einsum('pmrs, yr, ps, mr -> ym', h_tmp, lams, rdm1, rdm1)
        grad_l -= einsum('Smr, yr, Smr -> ym', vk, lams, rdm1) * 2.0
        ...
        grad_l -= einsum('pmr, ym, mp, r -> ym', v_pqr, lams, rdm1_tr, rdm1_diag_2)
        grad_l += einsum('pmr, ym, Spr, Smr -> ym', v_pqr, lams, rdm1, rdm1) * 2.0

        grad_l += einsum('mqr, yq, qm, r -> ym', v_pqr, lams, rdm1_tr, rdm1_diag_2)
        grad_l -= einsum('mqr, yq, Smr, Sqr -> ym', v_pqr, lams, rdm1, rdm1) * 2.0
        v_pqr = None

        # linear grad
        grad_l -= einsum('ypq, qp, m -> ym', G, rdm1_tr, rdm1_diag_2)
        grad_l += einsum('ypq, Spm, Sqm -> ym', G, rdm1, rdm1) * 2.0
