...
def take2d_tril(a, idx_list1, idx_list2=None, compact=False):
    ...
    if compact:
        ...
        res = a[idx].reshape(len(idx_list1), len(idx_list2))
    return res
...
def eri_idx(i, j, k, l, nao=None, eri_format='s1'):
    if eri_format == 's1':
        ...
        res = s4_idx(i, j, k, l, nao=nao)
    elif eri_format == 's8':
        res = s8_idx(i, j, k, l, nao=nao)
    else:
        raise ValueError
    return res
...
def format_idx(idx_list):
    ...
    """
    string = ''
    for (_, g) in it.groupby(enumerate(idx_list), lambda ix: ix[0] - ix[1]):
        g = list(g)
        if len(g) > 1:
